...
void printData(int S, int E)
{
    for (int i = 0; i < S; i++)
    {
        printf("\n| %8.2f \t |  %d \t\t | %10.2f \t | %12.2f \t | %6.2f \t|", tabla0.distance, E + i, tabla0.gVar, tabla0.gFuel[i], tabla0.gVar + tabla0.gFuel[i]);
        printf("\n---------------------------------------------------------------------------------");
        saveFile(i, E + i);
    }
...
{
    FILE *data;
    data = fopen("tabla de costos.txt", "a");

    if (data != NULL)
    {
        fprintf(data, "\n| %8.2f \t |  %d \t\t | %10.2f \t | %12.2f \t | %6.2f \t|", tabla0.distance, E, tabla0.gVar, tabla0.gFuel[X], tabla0.gVar + tabla0.gFuel[X]);
        fclose(data);
    }
    else
    {
